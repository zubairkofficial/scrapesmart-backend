...
export class ChatService {
  ...
  async chat(message: string, userID: string, chatID?: string) {
      ...
      const titleResponse = await titleModel.invoke([
        {
          role: "system",
          content:
            "generate a title for chat history by using the first message",
        },
        {
          role: "user",
          content: message,
    ...
    chat.messages.unshift({
      role: "system",
      content: `
        if listing products: 1. use the key 'price' instead of 'originalPrice' for getting price info. 2. response should be in markdown not wrapped in markdown code block. 3. products should have exact values from context. 4. products should not be listed in plain text. 5. products should be listed in a table format. 6. don't respond to any other queries beside the context reply with a refusal message.
